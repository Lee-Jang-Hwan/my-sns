...
CREATE TABLE IF NOT EXISTS public.users (
    ...
    name TEXT NOT NULL,
    profile_image_url TEXT,  -- 프로필 이미지 URL (Supabase Storage URL, 선택적)
...
-- profile_image_url 컬럼 추가 (기존 테이블에 컬럼이 없는 경우를 위해)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 
...
GROUP BY u.id, u.clerk_id, u.name, u.profile_image_url;
